...
namespace ZmBlog.Tests
{
    public abstract class ZmBlogTestBase : AbpIntegratedTestBase<ZmBlogTestModule>
    {
        ...
        protected void UsingDbContext(Action<ZmBlogDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<ZmBlogDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<ZmBlogDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<ZmBlogDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ZmBlogDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ZmBlogDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<ZmBlogDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ZmBlogDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<ZmBlogDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<ZmBlogDbContext, Task<T>> func)
        {
            T result;
